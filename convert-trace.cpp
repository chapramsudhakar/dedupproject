...
int main(int argc, char** argv) {
	...
	while (trace->readNextSegment(&pid, &op, &ts1, &ts2, &s)) {
		...
		for (i=0; i<s.seg_size; i++) {
			if ((t1 = searchWriteReq(s.seg_start + i)) == NULL) {
				t1 = (WriteReq *)malloc(sizeof(WriteReq));
				assert(t1 != NULL);
				t1->wr_ts = ts1 + i * incr;
				t1->wr_lba = s.seg_start + i;
				t1->wr_pid = pid;
				memcpy(t1->wr_hash, s.seg_hash[i], 16);
				insertWriteReqTS(t1);
				insertWriteReqHASH(t1);
			}
			else {
				if (op == OP_WRITE) {
					deleteWriteReqTS(t1);
		...
		//cout << s.seg_start << " " << s.seg_size << " " << trace->getCurCount() << endl;
...
#ifndef	HOME
	outputTrace("output.txt", 8);
#else
	outputTrace("output.txt", 1);
#endif
